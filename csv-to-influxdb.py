...
def loadCsv(
    ...
    with inputfile as csvfile:
        ...
        for row in reader:
            ...
            datetime_naive = datetime.datetime.fromtimestamp(
                int(row[timecolumn]) / 10e8
            )
            ...
            for f in fieldcolumns:
                ...
                if f in row:
                    if f == "power":
                        ...
                        v = str2bool(row[f]) if row[f] else None
                    else:
                        v = row[f] if row[f] else None
                fields[f] = v
            point = {
                "measurement": metric,
                "time": timestamp,
            ...
            if len(datapoints) % batchsize == 0:
                ...
                try:
                    ...
                    print(
                        "Wrote %d points, up to %s, response: %s"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", nargs="?", required=True, help="Input csv file."
    )
    ...
    parser.add_argument(
        "--ssl", action="store_true", default=False, help="Use HTTPS instead of HTTP."
    )

    parser.add_argument("-u", "--user", nargs="?", default="root", help="User name.")

    parser.add_argument("-p", "--password", nargs="?", default="root", help="Password.")
    ...
    parser.add_argument(
        "-b", "--batchsize", type=int, default=5000, help="Batch size. Default: 5000."
    )
